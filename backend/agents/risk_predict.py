...
class RiskPredictAgent:
    ...
    using historical patterns, terrain analysis, and weather correlation.
    """
    ...
    def _calculate_risk_score(self, signal: dict) -> dict:
        ...
        if risk_score >= 75:
            ...
            risk_level = "HIGH"
            action = f"EVACUATION ADVISORY — Prepare for immediate evacuation in {region.get('name', 'affected area')}"
        elif risk_score >= 30:
            risk_level = "MODERATE"
            action = f"ALERT — Monitor conditions closely in {region.get('name', 'affected area')}, prepare emergency kits"
        else:
            risk_level = "LOW"
    ...
    async def run_cycle(self):
        ...
        shared_state.update_agent_status(self.name, {
            ...
            "current_action": "Analyzing disaster signals for risk prediction",
        ...
        if not signals:
            shared_state.update_agent_status(self.name, {
                ...
                "state": "active",
                "messages_processed": self.messages_processed,
                "current_action": "No signals to process — monitoring",
                "uptime_seconds": time.time() - self.start_time,
            })
            return []
        ...
        shared_state.update_agent_status(self.name, {
            ...
            "state": "active",
            "messages_processed": self.messages_processed,
            "current_action": f"Assessed {len(risk_zones)} risk zones — {sum(1 for z in risk_zones if z['risk_level'] in ('HIGH', 'CRITICAL'))} high/critical",
            "uptime_seconds": time.time() - self.start_time,
        })
